			...
			.then((publishableKey) => {
				if (!publishableKey) {
					this.notificationService.setNotification({
						content:
							"Echec de la connexion entre notre serveur et Stripe",
						type: "failure"
					});
					return;
				}
				...
				const clientSecret = this.localStorageService.getSpecificItem(
				...
				stripe
					...
					.then(({ error, paymentIntent }) => {
						...
						this.paymentResultSection.innerHTML =
							this.createSuccessInterface();
						this.notificationService.setNotification({
							content: `Paiement réussi; ${paymentIntent.id}`,
							type: "success"
						});
						this.localStorageService.removeSpecificItem(
							"payment_intent_client_secret"
						);
						setTimeout(() => {
							this.onNavigate("");
