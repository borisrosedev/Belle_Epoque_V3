		...
		this.stripeService
			...
			.then(async () => {
				...
				const { error, clientSecret } =
					await this.stripeService.getClientSecretOrError({
						orderAmount: orderAmount * 100
					});
				if (error) {
					this.paymentErrorsSection.innerHTML = paragraph({
	...
	initializeStripeElements() {
		...
		new Promise((resolve) => {
			...
			.then(() => {
				...
				linkAuthenticationElement.mount(
		...
		this.stripe
			...
			.then((res) => {
				if (res.error) {
					...
					this.localStorageService.setSpecificItem(
